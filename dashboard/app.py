...
with tab1:
    ...
    p_min = int(df_props["price"].dropna().min()) if not df_props.empty else 0
    p_max = int(df_props["price"].dropna().max()) if not df_props.empty else 10000000
    price_min, price_max = st.slider("Price Range (₹)", min_value=p_min, max_value=p_max, value=(p_min, p_max))
...
with col1:
    df_pie = run_query("SELECT property_type, COUNT(*) as count FROM properties GROUP BY property_type")
...
with col2:
    df_pay = run_query("SELECT payment_mode, COUNT(*) as count FROM transactions GROUP BY payment_mode")
...
with col3:
    df_lst = run_query("SELECT status, COUNT(*) as count FROM listings GROUP BY status")
    fig3 = px.bar(df_lst, x="status", y="count", title="Listings by Status",
                  color="status", color_discrete_sequence=px.colors.sequential.Teal)
    st.plotly_chart(fig3, use_container_width=True)

with col4:
    df_leads = run_query("SELECT interest_level, COUNT(*) as count FROM leads GROUP BY interest_level")
    fig4 = px.bar(df_leads, x="interest_level", y="count", title="Leads by Interest Level",
                  color="interest_level", color_discrete_sequence=px.colors.sequential.Blues)
    st.plotly_chart(fig4, use_container_width=True)

# ── SCATTER PLOT ─────────────────────────────────────
st.subheader("💰 Price vs Area Analysis")
df_scatter = run_query("SELECT title, price, area_sqft, property_type, city FROM properties")
fig5 = px.scatter(df_scatter, x="area_sqft", y="price", color="property_type",
                  hover_data=["title", "city"], title="Property Price vs Area (sqft)",
...
""")
fig6 = px.line(df_trend, x="month", y="revenue", title="Monthly Revenue Trend",
               labels={"month": "Month", "revenue": "Revenue (₹)"},
               markers=True, color_discrete_sequence=["#2d4a3e"])
...
fig7 = px.imshow(df_pivot, title="Agent Interaction Heatmap",
